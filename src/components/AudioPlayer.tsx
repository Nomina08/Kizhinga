...
export function AudioPlayer({ embedded = false }: { embedded?: boolean }) {
      ...
      <div className="glass-panel p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-8">
          ...
          <div className="flex gap-3">
            <Button variant="primary" icon={playing ? Pause : Play} onClick={toggle}>
              {playing ? 'Пауза' : 'Слушать'}
            </Button>
            {playing && <Button variant="ghost" icon={Square} onClick={stop} aria-label="Стоп" />}
          </div>
        </div>
        <p className="mt-6 text-body-sm text-stone-600 dark:text-stone-400 leading-relaxed border-t border-stone-200/80 dark:border-stone-700/50 pt-6">
          {AUDIO_LEGEND_TEXT}
        </p>
      </div>
    </ScrollReveal>
  ...
  return (
    <section className="section-shell">
      <div className="container-premium max-w-3xl">{content}</div>
    </section>
  );
}
